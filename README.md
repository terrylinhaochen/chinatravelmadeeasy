...
The isolated backend lives in `supabase/`: RLS migrations, queue-backed submission contract, public `security_invoker` views, and seven Edge Function interfaces. `process-video-jobs` now claims durable queue messages and retrieves official TikTok oEmbed metadata; Instagram uses an explicitly configured approved Graph oEmbed endpoint and token. Successful metadata is archived and marked `metadata_ready`, but the video deliberately remains `processing` until evidence extraction and AMap/Apple place resolution pass their separate gates. Function entrypoints and the short-form-only production URL parser can be checked with `npm run test:supabase-functions`; static schema/function integration checks run with `npm run test:supabase-contract`; database RLS, privilege, rate-limit, deduplication, queue, and metadata-completion behavior are specified in `supabase/tests/backend_security_test.sql` for `supabase test db`. Production ingestion still needs a dedicated deployed project, scheduled worker invocation, AMap, Apple Maps Server, Turnstile, and approved platform credentials. The hourly Pages workflow materializes approved database records into static video/place pages when `CTME_SUPABASE_URL` and `CTME_SUPABASE_PUBLISHABLE_KEY` are configured. See `docs/video-ingestion-worker.md` and `docs/video-to-map-benchmark.md`; source checks are not an end-to-end backend certification.
