...
The next evidence gate is to collect 20 recent Xiaohongshu/Dianping submissions for Shanghai, resolve every place against AMap, and run the blind decision-change test against a category- and distance-matched English set. A smaller Seoul replication should then reverse the language relationship: compare English traveler sources with Korean-native sources to test whether the effect generalizes beyond China.
