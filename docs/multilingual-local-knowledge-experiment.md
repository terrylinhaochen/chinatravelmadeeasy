...
## Cross-market need-finding evidence

The [2026-07-15 public-evidence synthesis](./local-knowledge-need-finding-2026-07-15.md) codes 23 traveler discussions across China, South Korea, and Japan by workflow stage, workaround, consequence, and decision impact. China and South Korea show repeated manual identity repair and parallel-map behavior. Japan is the useful counterexample: local-language sources can change restaurant judgment and bookings even while Google Maps remains a viable execution layer. This makes the product wedge destination-specific rather than a universal “local recommendations” feed.

The evidence is problem discovery, not participant validation. The public study entry at `/research/local-lens/` now screens both city studies against the same primary-cohort criteria and keeps exploratory responses out of the primary denominator. Use the linked [recruitment and interview protocol](./local-lens-recruitment-interview-protocol.md) and [participant log](./local-lens-participant-log-template.csv) to collect the missing behavioral proof: a real added or displaced stop, its counterfactual, and a completed provider handoff.
